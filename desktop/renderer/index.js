...
const elements = {
  ...
  launchEnabled: document.getElementById('launch-enabled'),
...
function updateBadgeHint(badge) {
  if (!badge) {
    elements.badgeHint.textContent = '菜单栏数字为距下次提醒的剩余分钟数';
    return;
  }
  const label = ALARM_LABELS[badge.name] || '提醒';
  elements.badgeHint.textContent = `菜单栏：${label} · ${badge.minutes} 分钟后`;
}
...
function applySettings(settings) {
  ...
  elements.launchEnabled.checked = settings.launchAtLogin;
...
function collectSettings() {
  return {
    ...
    launchAtLogin: elements.launchEnabled.checked,
  };
}

async function saveSettings() {
  const response = await window.healthReminder.saveSettings(collectSettings());
  if (response) {
...
async function refreshStatus() {
  const response = await window.healthReminder.getStatus();
  if (response) {
    cachedStatus = response.status;
    applySettings(response.settings);
    updateCountdowns(cachedStatus);
...
function bindEvents() {
  ...
  elements.launchEnabled.addEventListener('change', saveAndRefresh);
  ...
  async function remindNow(alarmName, countdownEl) {
    ...
    try {
      const response = await window.healthReminder.remindNow(alarmName);
      if (response?.ok) {
...
window.healthReminder.onStatusChanged(() => {
  refreshStatus();
});
